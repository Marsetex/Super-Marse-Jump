...
public class Menu {
    ...
    static Boolean menu         = true, start = false, highscore = false, steuerung = false, credits = false, gameover = false, gewonnen = false;
    ...
    public void initMenuBilder() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            try {
                bImage = ImageIO.read(new File("menubilder" + (i + 1) + ".png"));
            } catch (IOException e) {
    ...
    public void initRollover() {
        ...
        for (int i = 0; i < 4; i++) {
            BufferedImage bImage = null;
            try {
                bImage = ImageIO.read(new File("rollover" + (i + 1) + ".png"));
            } catch (IOException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
